...
class LinearReasoner:
    ...
    def rank_texts(self, query_text: str, candidate_texts: List[str]) -> List[Tuple[str, float]]:
        ...
        ranked = self.rank(qv, cvs)
        return [(candidate_texts[cvs.index(c)], s) for c, s in ranked]
...
class PreAudit:
    ...
    CHECKS = [
        ...
        ("has_inputs",      lambda t: bool(t.get("raw")),
         "Empty input string"),
        ("no_injection",    lambda t: not re.search(
            r'[;|`$]|\.\./', t.get("raw", "")),
...
class ReasoningEngine:
    ...
    def __init__(self):
        self.algebra     = AlgebraicReasoner()
        self.linear      = LinearReasoner()
        self.quantum     = QuantumProbabilistic()
        self.pre_audit   = PreAudit()
        # DifferentialReasoner is built on-demand with a scorer
    ...
    def decide(
        ...
        # ── Step 4: Differential — fine-tune config ───────────────────────
        best_config = valid_configs[0] if valid_configs else {}
        if scorer_fn and len(valid_configs) > 1:
            dr = DifferentialReasoner(scorer_fn=lambda c: scorer_fn({**c, "skill": chosen_skill}))

            def neighbors_fn(cfg):
                """Flip one value in each dimension to create neighbors."""
                nbrs = []
                for k, v in variable_domains.items():
                    for alt in v:
                        if alt != cfg.get(k):
                            nbrs.append({**cfg, k: alt})
                return nbrs

            best_config = dr.ascend(best_config, neighbors_fn, steps=3)
            diff_score  = scorer_fn({**best_config, "skill": chosen_skill})
            breakdown.append({"step": "differential", "final_config": best_config,
                              "score": round(diff_score, 4)})
        else:
            breakdown.append({"step": "differential", "skipped": True})
